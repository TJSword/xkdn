        ...
        <div class="content-card">
          <div class="card-header-row nav-chart-header">
            <div class="chart-title-row">
              <h2 class="card-title no-margin">可转债策略 vs 可转债等权</h2>
...
<script setup lang="ts">
  ...
  const getChartGridOption = () => ({
      top: 52,
      left: isCompactChart() ? 8 : 28,
      right: isCompactChart() ? 24 : 36,
      bottom: 42,
      containLabel: true
  })
  ...
  const initChart = (xAxisData: any, strategyData: any, benchmarkData: any) => {
      ...
      const option = {
          ...
          xAxis: {
              ...
              axisLabel: { color: '#8392A5', hideOverlap: true, showMinLabel: true, showMaxLabel: true, formatter: formatChartDateAxisLabel }
          },
  ...
  const getlocalData = async () => {
      ...
      try {
          ...
          if (!res?.result?.success || !payload?.dateList?.length || !payload?.strategyData?.length) {
          ...
          showMessage?.(error instanceof Error ? error.message : '可转债策略数据加载失败', 'error')
      } finally {
...
<style scoped>
  ...
  .chart-scale-button.active { color: #17202a; background: #add8e6; }
  .chart-scale-button:focus-visible { outline: 2px solid rgb(173 216 230 / 65%); outline-offset: 1px; }

  @media (min-width: 769px) {
      .nav-chart-header { display: grid; grid-template-columns: minmax(0, 1fr) auto auto; column-gap: 0.75rem; }
      .nav-chart-header .chart-title-row { display: contents; }
  }

  @media (max-width: 768px) {
      .nav-chart-header .chart-title-row { justify-content: space-between; gap: 0.5rem; width: 100%; }
